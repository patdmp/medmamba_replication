...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
        ...
        self.patch_embed = PatchEmbed2D(patch_size=patch_size, in_chans=self.in_chans, embed_dim=self.embed_dim,
...
medmamba_t = VSSM(depths=[2, 2, 4, 2],dims=[96,192,384,768],num_classes=6, in_chans=3).to("cuda")
medmamba_s = VSSM(depths=[2, 2, 8, 2],dims=[96,192,384,768],num_classes=6, in_chans=3).to("cuda")
medmamba_b = VSSM(depths=[2, 2, 12, 2],dims=[128,256,512,1024],num_classes=6, in_chans=3).to("cuda")

data = torch.randn(1,3,224,224).to("cuda")

# print(medmamba_t(data))
# print(medmamba_s(data))
# print(medmamba_b(data))